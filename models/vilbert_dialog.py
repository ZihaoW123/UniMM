...
import pdb
from utils.losses import binary_ce_focal_loss, multi_ce_focal_loss, ghmc_loss
...
class BertEmbeddingsDialog(nn.Module):
    ...
    def forward(self, input_ids, sep_indices=None, sep_len=None, token_type_ids=None, position_ids=None):
...
class BertBiAttention(nn.Module):
    ...
    def forward(self, input_tensor1, attention_mask1, input_tensor2, attention_mask2, co_attention_mask=None,
                attended_all_tensor1=True, attended_all_tensor2=False):
        ...
        if (co_attention_mask is not None) and (not attended_all_tensor1):
        ...
        # if use_co_attention_mask:
        if (co_attention_mask is not None) and (not attended_all_tensor2):
    ...
    def forward(
        ...
        for v_layer_id, t_layer_id in zip(self.v_biattention_id, self.t_biattention_id):
            ...
            if count == 0 and self.in_batch_pairs:
                ...
                txt_attention_mask = txt_attention_mask.unsqueeze(1).expand(batch_size, batch_size, 1, 1,
            ...
            if count == 0 and self.FAST_MODE:
                txt_embedding = txt_embedding.expand(image_embedding.size(0), txt_embedding.size(1),
                                                     txt_embedding.size(2))
                txt_attention_mask = txt_attention_mask.expand(image_embedding.size(0), txt_attention_mask.size(1),
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        assert co_attention_mask.dim() == 3
        ...
        input_txt_shape = input_txt.size()
        ...
        if attention_mask.dim() == 3:
            ...
            raise ValueError(f"Wrong shape for txt input_ids (shape {input_txt.size()}) or attention_mask (shape {attention_mask.shape})")

        if image_attention_mask.dim() == 3:
            extended_image_attention_mask = image_attention_mask[:, None, :, :]
        elif image_attention_mask.dim() == 2:
            extended_image_attention_mask = image_attention_mask[:, None, None, :]
        else:
            raise ValueError(f"Wrong shape for img input_ids (shape {input_imgs.size()}) or attention_mask (shape {image_attention_mask.shape})")
        ...
        extended_attention_mask = extended_attention_mask.to(
            dtype=torch.float32  # torch.next(self.parameters()).dtype
        )  # fp16 compatibility
        ...
        extended_co_attention_mask = (1.0 - extended_co_attention_mask) * -10000.0
        # extended_co_attention_mask = co_attention_mask.unsqueeze(-1)

        embedding_output = self.embeddings(input_txt, token_type_ids=token_type_ids, sep_indices=sep_indices,
                                           sep_len=sep_len, position_ids=position_ids)
        ...
        # print('extended_image_attention_mask', extended_image_attention_mask.size())
        # print('extended_co_attention_mask', extended_co_attention_mask.size())
        #
        # print('input_txt', input_txt[2])
        # print('extended_co_attention_mask', extended_image_attention_mask[2,:,2])
        # print('extended_attention_mask', extended_attention_mask[2,:,0])
        # print('sep_indices', sep_indices[2])
        # for i in range(tmp):
        #     print('extended_attention_mask', i, extended_attention_mask[2, :, i, :256])
        # print('extended_image_attention_mask', extended_image_attention_mask[2])
        # for i in range(extended_co_attention_mask.shape[-2]):
        #     print('extended_co_attention_mask', i, extended_co_attention_mask[2, :, i, :tmp])
...
class BertForMultiModalPreTraining(BertPreTrainedModel):
    ...
    def forward(
        ...
        clamp_min = 1e-6 # 1e-6 if self.opt['fp16'] else 1e-20
        if masked_lm_labels is not None and next_sentence_label is not None and image_target is not None:
            ...
            if lm_weight is not None:
                lm_weight_view = lm_weight.view(-1)
                masked_lm_labels_view = masked_lm_labels.view(-1)
                prediction_scores_t_view = prediction_scores_t.view(-1, self.config.vocab_size)
                l_notnull = lm_weight_view > 0
                ul_notnull = lm_weight_view == -1
                l_prediction_scores_t = prediction_scores_t_view[l_notnull, :]
                ul_prediction_scores_t = prediction_scores_t_view[ul_notnull, :]

                log_l_probs = F.log_softmax(l_prediction_scores_t, dim=-1)
                log_u1_probs= torch.log(torch.clamp(1.0 - F.softmax(ul_prediction_scores_t, dim=-1), min=clamp_min))

                l_loss = F.nll_loss(log_l_probs, target=masked_lm_labels_view[l_notnull], ignore_index=-1, reduction='none')
                l_loss = l_loss * (lm_weight_view[l_notnull].float())
                ul_loss = F.nll_loss(log_u1_probs, target=masked_lm_labels_view[ul_notnull], ignore_index=-1, reduction='none')
                num_tokens = (lm_weight != 0).sum()
                # assert num_tokens == (masked_lm_labels != -1).sum()
                # assert len(l_loss) + len(ul_loss) == num_tokens
                masked_lm_loss = (l_loss.sum()+ul_loss.sum())/num_tokens
                # masked_lm_loss = F.cross_entropy(prediction_scores_t.view(-1, self.config.vocab_size), masked_lm_labels.view(-1),
                #                                      ignore_index=-1, reduction='none')
                # masked_lm_loss = (masked_lm_loss * lm_weight.view(-1)).sum() / (masked_lm_labels != -1).sum()
            ...
            if nsp_weight is None:
                nsp_weight = torch.FloatTensor([1.0, 1.0]).to(seq_relationship_score.device)
            nsp_weight = nsp_weight.squeeze().float()
            nsp_weight = nsp_weight/nsp_weight[0]

            # print('nsp_weight', nsp_weight)
            # print('alpha', nsp_weight[0])
            # next_sentence_loss = 10 * binary_ce_focal_loss(predict=seq_relationship_score[:, 0],
            #                                          target=1-next_sentence_label,
            #                                          gamma=2, alpha=nsp_weight[0],
            #                                          reduction='mean', eps=clamp_min)

            next_sentence_loss = F.cross_entropy(
                input=seq_relationship_score.view(-1, 2),
                target=next_sentence_label.view(-1),
                weight=nsp_weight, reduction='mean',
            )
            # total_loss = masked_lm_loss + next_sentence_loss + masked_img_loss
            return masked_lm_loss.unsqueeze(0), masked_img_loss.unsqueeze(0), next_sentence_loss.unsqueeze(
                0), sequence_output_t, prediction_scores_t, seq_relationship_score
        else:
            return prediction_scores_t, prediction_scores_v, seq_relationship_score, sequence_output_t, all_attention_mask
